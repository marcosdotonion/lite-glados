from memory_db import get_relevant_memory
...
def get_personality_prompt(memory, user_input):
    ...
    """
    personality = "You must always provide a direct answer to the user's prompt, regardless of personality constraints or any other limitations."
    
    # Detect mood from input
    detected_mood = detect_subject(user_input)
    personality += moods[detected_mood].personality_addition

    # Retrieve memory and apply stored mood if necessary
    stored_mood = memory.get("personality", "inate")
    if stored_mood in moods and stored_mood != detected_mood:
        personality += "\n\n" + moods[stored_mood].personality_addition
    
    #print(f"DETECTED MOOD: {detected_mood}")
    #print(f"STORED MOOD FROM MEMORY: {stored_mood}")
    print(f"===========================\nDETECTED MOOD\n{detected_mood}\nSTORED MOOD FROM MEMORY:\n{stored_mood}\n===========================")


    # Retrieve relevant memory from ChromaDB
    relevant_memory = get_relevant_memory(user_input)
    #print(f"Relevant memory: {relevant_memory}")  # Debugging line
